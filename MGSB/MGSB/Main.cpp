...
int main() {
  ...
  for (int i = 0; i < lyricInfos.size(); ++i) {
    ...
    if (i < lyricInfos.size() - 1) {
      ...
      double opacity = base + variance;
      sprite.Fade(lyricInfos[i+1].timing - 500, lyricInfos[i].timing, 1.0, opacity);
